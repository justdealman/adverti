;(function($, window, document, undefined){
    var options = {
            ...
            tpl : {
                ...
                wrapper: '<div class="prodigal_wrapper prodigal_wrapper_custom">' +
                ...
                gallery: '<div class="prodigal_gallery_wrapper"><div class="prodigal_gallery_loader"></div>' +
                    '<div class="prodigal_gallery_left"></div>' +
                    '<div class="prodigal_gallery_middle"></div>' +
                    '<div class="prodigal_gallery_right"></div>' +
                    '<div class="prodigal_gallery_left-button"></div>' +
                    '<div class="prodigal_gallery_right-button"></div>' +
                    '</div>',
                sorter: '<div class="prodigal_sorter"><select>' +
        ...
        methods = {
            ...
            open: function(e){
                ...
                if(options.isActive == false){
                    ...
                    var leftButton = options._wrapper.find('.prodigal_gallery_left-button');
                    var rightButton = options._wrapper.find('.prodigal_gallery_right-button');
                    leftButton.hide();
                    rightButton.hide();

                    var thumbnailTitle;
                    $(document).find('.prodigal-thumbs').each(function(i, obj){//показываем стрелки только если картинок больше 1
                        if ( i > 0 ) {
                            leftButton.show();
                            rightButton.show();
                        }
                        ...
                        image = $('<div class="image_item"><a class="prodigal_thumb" href="'+media_url + '"><img src="'+thumbnail+'" title="' + thumbnailTitle + '"/></a></div>');
                        ...
                        options._wrapper.find('.prodigal_gallery_right').append(image.on('click', {'element': $(this)}, imageClickHandler));
...
//                            image.trigger('click');
                            if ( navigator.userAgent.indexOf('Firefox') != -1 ) {
                                setTimeout(function() {
                                    image.parent().find('.image_item.prodigal_img_selected').triggerHandler('click');
                                }, 200);
                            }
                    ...
                    rightButton.on('click', function() {
                        if (options._wrapper.find('.prodigal_img_selected').next().length > 0) {
                            options._wrapper.find('.prodigal_img_selected').next().triggerHandler('click');
                        } else {
                            options._wrapper.find('.prodigal_img_selected').siblings().first().triggerHandler('click');
                        }
                    });
                    leftButton.on('click', function() {
                        if (options._wrapper.find('.prodigal_img_selected').prev().length > 0) {
                            options._wrapper.find('.prodigal_img_selected').prev().triggerHandler('click');
                        } else {
                            options._wrapper.find('.prodigal_img_selected').siblings().last().triggerHandler('click');
                        }
                    });
                }
            ...
            resize: function(resizeImage){
                ...
                gallery_left.css({
                    height: (wrapperHeight-closeHeight)-(parseInt(gallery_left.css('paddingTop'))+options._wrapper.find('.prodigal_gallery_right').outerHeight(true)+parseInt(gallery_left.css('paddingBottom'))+parseInt($('.prodigal_gallery_middle').outerHeight())),
                    lineHeight: wrapperHeight-closeHeight+"px",
                    width: (options._wrapper.width())-(parseInt(gallery_left.css('paddingLeft'))+parseInt(gallery_left.css('paddingRight')))
                });
         ...
         */
            ...
            if($(this).data('prodigal-media-type') !== null && $(this).data('prodigal-media-type') == 'video'){
                ...
                options._wrapper.find('.prodigal_large_preview').load(function(){
                    methods.resize();
                    options._wrapper.find('.prodigal_gallery_loader').fadeOut();
            ...
            $('.prodigal_gallery_middle').text( $(this).find('img').attr('title') );

            options._wrapper.find('.prodigal_gallery_loader').show();

            //place for event after
            setTimeout(function() {
                if (e.data) e.data.element.trigger('afterOpen.prodigal');
            }, 200);
